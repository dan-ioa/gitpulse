...
> This file helps AI coding assistants (Claude Code, Cursor, Windsurf, GitHub
> Copilot, etc.) write correct CrewAI code.
>
> **Project Overview below is specific to this repo and maintained by hand.**
> Everything from "Version & Freshness Requirements" onward is the stock CrewAI
> reference generated by `crewai create` — to update it, copy the latest version
> from the [CrewAI repository](https://github.com/crewAIInc/crewAI).

---

## Project Overview — Daily GitHub AI Tools Digest

This project is a 3-agent CrewAI crew that runs daily to discover the best new open-source AI tools on GitHub and send a rendered HTML digest as a Resend broadcast to every contact in an audience.

### Agents

| Agent | Role | Tools |
|---|---|---|
| `daily_trend_researcher` | Searches GitHub Trending, GitHub Search, Hacker News, and Product Hunt for new AI repos | `SerperDevTool`, `GithubSearchTool` (if `GITHUB_TOKEN` set) |
| `content_creator_and_summarizer` | Formats research into a JSON object holding the subject, welcome message, and project list | none |
| `email_reporter` | Calls `send_html_email` with the project list to deliver the digest | `SendHtmlEmailTool` |

### Tasks (sequential)

1. **`research_daily_trends`** → structured list of 8-12 GitHub repos with confirmed URLs
2. **`create_headlines_and_summaries`** → raw JSON object with `subject`, `welcome_message`, and a `projects` array (no HTML, no prose)
3. **`send_daily_trends_email_report`** → calls `SendHtmlEmailTool`, which validates URLs, renders the fixed template, and sends via Resend

### Custom tool — `SendHtmlEmailTool`

Defined in `src/gitpulse/tools/custom_tool.py`.

- Input: `subject`, `welcome_message`, `projects: list[dict]` — there is no recipient argument; delivery targets `RESEND_AUDIENCE_ID`
- Each project dict requires `name` and `github_url`; `owner_repo`, `description`, `language` and `stars` are optional and fall back to defaults
- Call it **exactly once** per run with the complete project list — never once per project
- URL validation: projects without a valid GitHub URL are dropped before sending
- HTML rendering: fixed Python template — agents do NOT generate HTML
- Delivery: one Resend **broadcast** (`RESEND_API_KEY` + `RESEND_AUDIENCE_ID`), sent from `RESEND_FROM_EMAIL`
- A markdown report is written to `reports/` on every run, whether or not the send succeeds

### Key environment variables

| Variable | Purpose |
|---|---|
| `LLM_PROVIDER` | `google` or `local` |
| `GOOGLE_API_KEY` | Google AI Studio key (formats vary: `AIza…`, `AQ.…`) |
| `MODEL` | e.g. `gemini/gemini-2.5-flash` |
| `OPENAI_API_KEY` | Same as `GOOGLE_API_KEY` — satisfies tools that check for it |
| `OPENAI_BASE_URL` | `https://generativelanguage.googleapis.com/v1beta/openai/` |
| `GITHUB_TOKEN` | No-scope PAT for `GithubSearchTool` (optional) |
| `SERPER_API_KEY` | Web search |
| `RESEND_API_KEY` | Email delivery |
| `RESEND_AUDIENCE_ID` | **Required.** Audience the digest is broadcast to |
| `RESEND_FROM_EMAIL` | Sender address; defaults to `The Git Pulse <onboarding@resend.dev>` |

### Conventions in this project

- **No HTML generation by agents.** The `SendHtmlEmailTool` owns the email template.
- **JSON output from content creator.** The `create_headlines_and_summaries` task outputs a raw JSON object — not HTML, not markdown.
- **URL validation is enforced in the tool**, not in task prompts alone.
- **`LLM_PROVIDER=google`** uses CrewAI's native Gemini provider (`crewai[google-genai]`). Switch to `local` for LM Studio.
- **`OPENAI_API_KEY` + `OPENAI_BASE_URL`** are set to Google AI Studio values so that tools requiring OpenAI (e.g. `GithubSearchTool`) route through Google's OpenAI-compatible endpoint.
